...
def benchmark_inference(
    ...
    """Run the full inference benchmark suite for a trained Q-Drifting agent.

    Args:
        agent:       Trained QDriftAgent (sample_actions must be jit-compiled).
        example_obs: Representative observation array, shape (B, obs_dim).
                     Values don't matter; only the shape is used.
        action_dim:  Dimensionality of the action space.
        n_warmup:    Warmup calls before timing begins (excludes JIT compile).
        n_trials:    Number of timed trials to average over.

    Returns:
        Dict matching the JSON schema described in the module docstring.
    """
    rng_seq = _RNGSequence(0)
    obs_dim = example_obs.shape[-1]
    ...
    # 1. Single-observation latency
    # ------------------------------------------------------------------ #
    print('  [1/3] Single-observation latency ...')
    obs_1 = jnp.array(example_obs[:1])  # (1, obs_dim)
