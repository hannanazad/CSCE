...
def make_env_and_datasets(env_name, frame_stack=None, action_clip_eps=1e-5, dataset_dir=None):
    ...
    Args:
        ...
        dataset_dir: Path to pre-downloaded OGBench datasets (avoids downloading).
    ...
    if 'singletask' in env_name:
        ...
        for ds in [train_dataset, val_dataset]:
            if ds is not None:
                if 'rewards' not in ds:
                    ds['rewards'] = np.zeros(len(ds['actions']), dtype=np.float32)
                if 'masks' not in ds:
        ...
        # General OGBench environment (antmaze, humanoidmaze, etc.)
        env, train_dataset, val_dataset = ogbench.make_env_and_datasets(env_name, **ogbench_kwargs)
        eval_env = ogbench.make_env_and_datasets(env_name, env_only=True)
        env = EpisodeMonitor(env)
        eval_env = EpisodeMonitor(eval_env)
        # Goal-conditioned OGBench datasets lack 'rewards' and 'masks'; add them.
        for ds in [train_dataset, val_dataset]:
            if ds is not None:
                if 'rewards' not in ds:
                    ds['rewards'] = np.zeros(len(ds['actions']), dtype=np.float32)
